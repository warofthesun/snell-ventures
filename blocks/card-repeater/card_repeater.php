...
if ( function_exists( 'client_block_inserter_preview' ) && client_block_inserter_preview( $block, 'card-repeater' ) ) {
	return;
}
...
if ($is_editor_context && empty($rows) && empty($block_data['inserter_preview'])) {
  ...
  echo '    <strong>' . esc_html__( 'Card Repeater', 'client_theme' ) . '</strong><br>';
  echo '    ' . esc_html__( 'Add one or more cards in the block settings or click the pencil icon ^ to edit in place.', 'client_theme' );
  echo '  </div>';
  echo '</div>';
  return;
}
