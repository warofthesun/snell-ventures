...
<div id="inner-footer" class="wrap row">
	<div class="col-xs-12 col-md-6 footer__logo-nav">
		...
		if ( ! empty( $social_platforms ) ) :
			?>
		<nav class="footer__social" aria-label="<?php esc_attr_e( 'Social media', 'client_theme' ); ?>">
			...
			foreach ( $social_platforms as $row ) {
				...
				if ( $icon_html !== '' ) {
					echo '<li class="footer__social-item"><a href="' . esc_url( $url ) . '" class="footer__social-link" target="' . esc_attr( $target ) . '" rel="noopener noreferrer"' . ( $title ? ' title="' . esc_attr( $title ) . '"' : '' ) . '>' . $icon_html . '<span class="screen-reader-text">' . esc_html( $title ? $title : __( 'Social link', 'client_theme' ) ) . '</span></a></li>';
				}
			}
			?>
			</ul>
	...
	<div class="footer__copyright">&copy; <?php echo date('Y'); ?> <?php bloginfo( 'name' ); ?>.</div>
		...
		<?php // Main scripts/styles enqueued from library/starter.php (client_scripts_and_styles) ?>
		<?php wp_footer(); ?>
