 ...
 *
 * Expects $client_featured_post_id (int) in scope from index.php.
 *
 * @package client_theme
 */

if ( empty( $client_featured_post_id ) ) {
	return;
}

$post = get_post( (int) $client_featured_post_id );
...
$chip_terms = function_exists( 'client_get_post_chip_terms_for_links' )
	? client_get_post_chip_terms_for_links( $post_id, 4 )
	: array();
...
<section class="c-storiesFeatured<?php echo esc_attr( $featured_mod ); ?>" aria-labelledby="c-storiesFeatured-title">
	<div class="c-storiesFeatured__inner wrap row">
		<div class="c-storiesFeatured__content col-xs-12 col-md-6">
			...
			<?php endif; ?>
			<a class="c-storiesFeatured__cta" href="<?php echo esc_url( $permalink ); ?>"><?php esc_html_e( 'Read now', 'client_theme' ); ?></a>
		</div>
		<?php if ( $image_id ) : ?>
