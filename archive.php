						...
						<main id="main" class="col-xs-12 col-sm-8 col-lg-9 " role="main" itemscope itemprop="mainContentOfPage" itemtype="http://schema.org/Blog">
							...
							<article id="post-<?php the_ID(); ?>" <?php post_class( '' ); ?> role="article">
								...
								<header class="entry-header article__header">
									...
									<p class="byline entry-meta vcard">
										<?php printf( __( 'Posted', 'client_theme' ).' %1$s %2$s',
                  							     /* the time the post was published */
                  							     '<time class="updated entry-time" datetime="' . get_the_time('Y-m-d') . '" itemprop="datePublished">' . get_the_time(get_option('date_format')) . '</time>',
                       								/* the author of the post */
                       								'<span class="by">'.__('by', 'client_theme').'</span> <span class="entry-author author" itemprop="author" itemscope itemptype="http://schema.org/Person">' . get_the_author_link( get_the_author_meta( 'ID' ) ) . '</span>'
                    							); ?>
									</p>
							...
							<?php else : ?>
									...
									<article id="post-not-found" class="hentry ">
										...
										<footer class="article__footer">
												<p><?php _e( 'This is the error message in the archive.php template.', 'client_theme' ); ?></p>
										</footer>
									</article>
