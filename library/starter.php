...
function client_head_cleanup() {
	...
	add_filter( 'style_loader_src', 'client_remove_wp_ver_css_js', 9999 );
	// remove Wp version from scripts
	add_filter( 'script_loader_src', 'client_remove_wp_ver_css_js', 9999 );

} /* end client head cleanup */
...
// remove WP version from scripts
function client_remove_wp_ver_css_js( $src ) {
	if ( strpos( $src, 'ver=' ) )
...
function client_remove_wp_widget_recent_comments_style() {
...
function client_remove_recent_comments_style() {
...
function client_gallery_style($css) {
...
function client_asset_version( $path_relative_to_theme ) {
...
function client_needs_scroll_reveal() {
	return is_front_page() || is_home() || is_archive();
}

// loading scripts and styles with versioning and conditional third-party assets
function client_scripts_and_styles() {
	if ( is_admin() ) {
		return;
	}

	$theme_uri = get_stylesheet_directory_uri();
	$version   = client_asset_version( 'library/css/style.css' );
	$js_version = client_asset_version( 'library/js/scripts.js' );

	// Main stylesheet (always)
	wp_register_style( 'client-stylesheet', $theme_uri . '/library/css/style.css', array(), $version, 'all' );
	...
	wp_register_script( 'client-js', $theme_uri . '/library/js/scripts.js', $script_deps, $js_version, true );
	wp_enqueue_script( 'client-js' );

	// Modernizr: only if a script or style depends on it (filter to true to load)
	$use_modernizr = apply_filters( 'client_load_modernizr', false );
	if ( $use_modernizr ) {
		wp_register_script( 'client-modernizr', $theme_uri . '/library/js/libs/modernizr.custom.min.js', array(), '2.5.3', true );
		wp_enqueue_script( 'client-modernizr' );
	}
}
...
function client_theme_support() {
	...
	register_nav_menus(
		array(
			'main-nav' => __( 'The Main Menu', 'client_theme' ),   // main nav in header
			'footer-links' => __( 'Footer Links', 'client_theme' ) // secondary nav in footer
		)
	);
...
function client_related_posts() {
	echo '<ul id="client-related-posts">';
	global $post;
	...
	if($tags) {
		...
		if($related_posts) {
			...
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'client_theme' ) . '</li>'; ?>
		<?php }
	}
	wp_reset_postdata();
	echo '</ul>';
} /* end client related posts function */
...
function client_filter_ptags_on_images($content){
...
function client_excerpt_more($more) {
	...
	return '...  <a class="excerpt-read-more" href="'. get_permalink( $post->ID ) . '" title="'. __( 'Read ', 'client_theme' ) . esc_attr( get_the_title( $post->ID ) ).'">'. __( 'Read more &raquo;', 'client_theme' ) .'</a>';
}
