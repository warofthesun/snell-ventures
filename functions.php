...
// Theme core: enqueue, head cleanup, theme support, pagination, etc. (library/starter.php)
 ...
 * and what the server allows (ini_set may be disabled; use php.ini / host panel if uploads still fail).
 */
...
add_filter( 'upload_size_limit', 'client_upload_size_limit', 999, 3 );
function client_upload_size_limit( $size, $u_bytes, $p_bytes ) {
	$u_bytes = wp_convert_hr_to_bytes( ini_get( 'upload_max_filesize' ) );
	$p_bytes = wp_convert_hr_to_bytes( ini_get( 'post_max_size' ) );
	return (int) min( CLIENT_MAX_UPLOAD_BYTES, $u_bytes, $p_bytes );
}

/**
 * Register ACF blocks (single source of truth).
 */
function client_register_acf_blocks() {
  $block_dirs = glob( __DIR__ . '/blocks/*', GLOB_ONLYDIR );
  if ( ! $block_dirs ) {
    return;
  }
  foreach ( $block_dirs as $dir ) {
    // register_block_type( $path ) reads block.json from that folder; skip incomplete dirs.
 ...
 */
function client_headline_tag_and_class( $headline_size, $block_class = '' ) {
 ...
 */
function client_content_group_classes( $args = array() ) {
 ...
 */
function client_resolve_blog_featured_post_id() {
	static $cached = null;
	if ( null !== $cached ) {
		return $cached;
	}
 ...
 * @return int Post ID or 0.
 */
function client_get_blog_featured_post_id() {
	if ( ! is_home() || is_front_page() ) {
		return 0;
	}
	return client_resolve_blog_featured_post_id();
}
 ...
 */
function client_blog_index_pre_get_posts( $query ) {
	if ( is_admin() || ! $query->is_main_query() || ! $query->is_home() || $query->is_front_page() ) {
		return;
	}
	$fid = client_resolve_blog_featured_post_id();
	if ( $fid > 0 ) {
	...
	$sb_cfg    = client_get_blog_index_sidebar_config();
...
add_action( 'pre_get_posts', 'client_blog_index_pre_get_posts', 5 );
 ...
 */
function client_get_blog_index_sidebar_config() {
	static $cached = null;
	if ( null !== $cached ) {
		return $cached;
	}
 ...
 * @return string[]
 */
function client_body_class_stories_index( $classes ) {
	if ( is_home() && ! is_front_page() ) {
		$classes[] = 'client-stories-index';
		$sb        = client_get_blog_index_sidebar_config();
		if ( ! empty( $sb['active'] ) ) {
			$classes[] = 'client-blog-index-has-sidebar';
			$classes[] = 'client-blog-index-sidebar-' . sanitize_html_class( $sb['position'] );
		}
	}
	return $classes;
}

add_filter( 'body_class', 'client_body_class_stories_index' );

/**
 * Blog index: scroll #client-stories-header to 50px from top after paginated loads.
 */
function client_enqueue_blog_stories_scroll_script() {
	if ( ! is_home() || is_front_page() ) {
		return;
	}
	$theme_uri = get_stylesheet_directory_uri();
	$path      = get_template_directory() . '/library/js/client-blog-stories-scroll.js';
	$ver       = file_exists( $path ) ? (string) filemtime( $path ) : null;
	wp_enqueue_script(
		'client-blog-stories-scroll',
		$theme_uri . '/library/js/client-blog-stories-scroll.js',
		array(),
		$ver,
		true
	);
}
add_action( 'wp_enqueue_scripts', 'client_enqueue_blog_stories_scroll_script', 20 );
 ...
 * @return WP_Term[]
 */
function client_get_post_chip_terms_for_links( $post_id, $limit = 4 ) {
	$post_id = (int) $post_id;
 ...
 */
function client_render_blog_post_card( $p, $args = array() ) {
	...
	$chip_terms = client_get_post_chip_terms_for_links( $post_id, 3 );
	?>
	<article class="c-postCard c-postCard--split">
		...
		<div class="c-postCard__body">
			...
			<?php if ( ! empty( $args['show_cta'] ) ) : ?>
				<a class="c-postCard__cta" href="<?php echo esc_url( $permalink ); ?>"><?php esc_html_e( 'read more', 'client_theme' ); ?></a>
			<?php endif; ?>
		</div>
	</article>
	<?php
 ...
 * - Single company: dedicated company hero (Figma Hero/Companies/Post Type/Single); sidebar fields + Industries taxonomy.
...
function client_get_hero_config() {
  ...
  if ( is_post_type_archive( 'tribe_events' ) && ! is_singular( 'tribe_events' ) ) {
    ...
    $bg_image  = function_exists( 'get_field' ) ? client_get_events_option( 'events_hero_background_image' ) : 0;
    if ( is_array( $bg_image ) && ! empty( $bg_image['ID'] ) ) {
      $bg_image = (int) $bg_image['ID'];
    } elseif ( ! is_numeric( $bg_image ) ) {
      $bg_image = 0;
    }
    $headline = function_exists( 'get_field' ) ? client_get_events_option( 'events_hero_headline_text' ) : '';
  ...
  // Single company: Figma company hero (featured image, title, Industries, sidebar stats).
  if ( is_singular( 'company' ) ) {
    ...
    if ( $terms && ! is_wp_error( $terms ) ) {
      $industry = implode( ', ', wp_list_pluck( $terms, 'name' ) );
    }

    $years_in_business     = function_exists( 'get_field' ) ? get_field( 'years_in_business', $post_id ) : '';
    $years_in_stewardship  = function_exists( 'get_field' ) ? get_field( 'years_in_stewardship', $post_id ) : '';
    $total_employees       = function_exists( 'get_field' ) ? get_field( 'total_employees', $post_id ) : '';
    ...
    $config = array(
      ...
      'data' => array(
        ...
        'years_in_stewardship' => $years_in_stewardship,
 ...
 */
function client_get_related_posts( $post_id, $post_type, $limit = 3 ) {
  $post_id   = (int) $post_id;
 ...
 */
function client_get_related_posts_settings( $post_type = 'post' ) {
  $post_type = $post_type ? $post_type : 'post';
  $key       = sanitize_key( $post_type );
  $defaults   = array(
    'post'         => __( 'Related Posts', 'client_theme' ),
    'tribe_events' => __( 'Related Events', 'client_theme' ),
  );
  $heading = isset( $defaults[ $key ] ) ? $defaults[ $key ] : __( 'Related Posts', 'client_theme' );
  $bg_id   = 0;
  $bg_url  = '';
 ...
 */
function client_get_event_external_url( $event_id ) {
 ...
 * @return bool
 */
function client_events_require_external_url() {
  if ( ! function_exists( 'get_field' ) ) {
    return false;
  }
 ...
 */
function client_filter_tribe_event_links( $url, $post, $leavename, $sample ) {
  if ( ! $post || $post->post_type !== 'tribe_events' ) {
    return $url;
  }
  $external = client_get_event_external_url( $post->ID );
  return $external ? $external : $url;
}
add_filter( 'post_type_link', 'client_filter_tribe_event_links', 10, 4 );
...
if ( function_exists( 'tribe_get_event_link' ) ) {
   ...
   * @return string
   */
  function client_filter_tribe_get_event_link( $link, $event_id ) {
    $external = client_get_event_external_url( $event_id );
    return $external ? $external : $link;
  }
  add_filter( 'tribe_get_event_link', 'client_filter_tribe_get_event_link', 10, 2 );
}
...
function client_setup() {
  ...
  load_theme_textdomain( 'client_theme', get_template_directory() . '/library/translation' );
  ...
  add_filter( 'wp_head', 'client_remove_wp_widget_recent_comments_style', 1 );
  // clean up comment styles in the head
  add_action( 'wp_head', 'client_remove_recent_comments_style', 1 );
  // clean up gallery output in wp
  add_filter( 'gallery_style', 'client_gallery_style' );

  // enqueue base scripts and styles
  add_action( 'wp_enqueue_scripts', 'client_scripts_and_styles', 999 );
  // launching this stuff after theme setup
  client_theme_support();

  // adding sidebars to Wordpress (these are created in functions.php)
  add_action( 'widgets_init', 'client_register_sidebars' );

  // cleaning up random code around images
  add_filter( 'the_content', 'client_filter_ptags_on_images' );
  // cleaning up excerpt
...
// Thumbnail sizes
add_image_size( 'client-thumb-600', 600, 150, true );
add_image_size( 'client-thumb-300', 300, 100, true );
...
<?php the_post_thumbnail( 'client-thumb-300' ); ?>
for the 600 x 150 image:
<?php the_post_thumbnail( 'client-thumb-600' ); ?>

You can change the names and dimensions to whatever
you like. Enjoy!
*/

add_filter( 'image_size_names_choose', 'client_custom_image_sizes' );

function client_custom_image_sizes( $sizes ) {
    return array_merge( $sizes, array(
        'gallery-image' => __('Gallery Image'),
        'hero-bg' => __('Hero background (1920×1080)'),
        'client-thumb-600' => __('600px by 150px'),
        'client-thumb-300' => __('300px by 100px'),
...
function client_google_maps_api_key() {
	...
	return (string) get_option( 'client_google_maps_api_key', '' );
}

function my_acf_init() {
	acf_update_setting( 'google_api_key', client_google_maps_api_key() );
}
...
function client_register_sidebars() {
	register_sidebar(array(
		'id' => 'sidebar1',
		'name' => __( 'Sidebar 1', 'client_theme' ),
		'description' => __( 'The first (primary) sidebar.', 'client_theme' ),
	...
	register_sidebar(array(
		...
		'name' => __( 'Header', 'client_theme' ),
		'description' => __( 'Widget area in the header (right of the navigation).', 'client_theme' ),
	...
	register_sidebar(array(
		'id' => 'sidebar2',
		'name' => __( 'Sidebar 2', 'client_theme' ),
		'description' => __( 'The second (secondary) sidebar.', 'client_theme' ),
		'before_widget' => '<div id="%1$s" class="widget %2$s">',
		'after_widget' => '</div>',
		'before_title' => '<h4 class="widgettitle">',
		'after_title' => '</h4>',
...
// Comment Layout
function client_comments( $comment, $args, $depth ) {
    ...
    <article  class="cf">
      <header class="comment-author vcard">
        ...
        */
        ...
        <?php printf(__( '<cite class="fn">%1$s</cite> %2$s', 'client_theme' ), get_comment_author_link(), edit_comment_link(__( '(Edit)', 'client_theme' ),'  ','') ) ?>
        <time datetime="<?php echo comment_time('Y-m-j'); ?>"><a href="<?php echo htmlspecialchars( get_comment_link( $comment->comment_ID ) ) ?>"><?php comment_time(__( 'F jS, Y', 'client_theme' )); ?> </a></time>

      </header>
      <?php if ($comment->comment_approved == '0') : ?>
        <div class="alert alert--info">
          <p><?php _e( 'Your comment is awaiting moderation.', 'client_theme' ) ?></p>
        </div>
      <?php endif; ?>
 ...
 */
function client_enqueue_tribe_events_overrides() {
	if ( ! is_post_type_archive( 'tribe_events' ) && ! is_singular( 'tribe_events' ) ) {
		return;
	}
...
function client_events_hero_and_intro_before_html( $before ) {
	...
	$hero_config = client_get_hero_config();
	$out        = '';
...
add_filter( 'tribe_events_before_html', 'client_events_hero_and_intro_before_html', 10, 1 );
...
function client_fonts_preconnect( $urls, $relation_type ) {
  if ( 'preconnect' !== $relation_type || ! apply_filters( 'client_enqueue_google_fonts', false ) ) {
    return $urls;
  }
...
add_filter( 'wp_resource_hints', 'client_fonts_preconnect', 10, 2 );
 ...
 */
function client_register_post_settings_options_page() {
...
add_action( 'acf/init', 'client_register_acf_events_settings' );
  ...
  add_filter('block_categories_all', function ($categories, $editor_context) {
    ...
    $client_category = [
      'slug'  => 'theme-blocks',
      'title' => __( 'Theme Blocks', 'client_theme' ),
      'icon'  => null,
    ];

    // Put it at the top
    array_unshift($categories, $client_category);
